...
namespace ICS_Tech
{
    public partial class Messages : Form
    {
        ...
        public Messages(string manager)
        {
            InitializeComponent();
        ...
        private string GetSQLOrdered()
        {
            ...
            ///сначала подготавливаем общую информацию о выборке
            ///поля и таблицу для выборки
            StringBuilder sSQL = new StringBuilder("SELECT * FROM otrabmail");
            ///если поле textBoxFamily не пустое,то делаем выборку по полю FM
            //if (textBoxFamily.Text != "") sSQL.Append(" AND name LIKE '%").Append(textBoxFamily.Text).Append("%'");
            ///если поле textBoxName не пустое,то делаем выборку по полю IM
            //if (textBoxName.Text != "") sSQL.Append(" AND opis LIKE '%").Append(textBoxName.Text).Append("%'");
        ...
        private void ZakazOt_Click(object sender, EventArgs e)
        {
            ...
            if (gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "id").ToString() != "")
            {
                            ...
                            "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    StringBuilder sSQL = new StringBuilder("INSERT INTO otrabmail (mail,fio,text,manager,data) VALUES (");
                    sSQL.Append("'").Append(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "mail").ToString()).Append("',");
                    sSQL.Append("'").Append(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "fio").ToString()).Append("',");
                    sSQL.Append("'").Append(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "letter").ToString()).Append("',");
                    ...
                    String sConnectionString = "Database=ics-tech2;Data Source=195.64.225.66;User Id=ics-tech2;Password=SheytUgnud;charset=utf8;";
                    ///выполняем SQL-запрос
                    MySqlLib.MySqlData.MySqlExecute.SqlNoneQuery("DELETE FROM mails WHERE id=" + gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "id").ToString(), sConnectionString);
        ...
        private void Otvet_Click(object sender, EventArgs e)
        {
            ...
            E_mail frm = new E_mail(gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "mail").ToString(), gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "fio").ToString(), gridView1.GetRowCellValue(gridView1.FocusedRowHandle, "letter").ToString());
            frm.Owner = this;
